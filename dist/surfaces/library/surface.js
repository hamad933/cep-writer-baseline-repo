...
export function bindLibrarySurface({commands,structured,libraryRuntime=null,workspace=null}={}){
  ...
  const ids=[
    ...
    register(commands,'library.save','Save Library document',payload=>structured.commit(payload),payload=>admitted(structured.transactionDescriptor().persistedBoundaryConfigured?structured.availability('document.commit',payload):unavailable('SAVE_BOUNDARY_UNAVAILABLE','A real Library persistence boundary is not bound'))),
