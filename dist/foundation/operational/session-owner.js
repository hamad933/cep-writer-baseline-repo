...
export const OPERATIONAL_SESSION_CONTRACT=Object.freeze({
  ...
  maturity:'BOUNDARY_ONLY_BELOW_M6_UNLESS_SECOND_REAL_PROVIDER_IS_PROVEN'
});
...
export class OperationalSessionOwner {
  ...
  detachWindow({url=null}={}){const tab=this.activeTab();if(!tab)throw Error('OPERATIONAL_DETACH_REQUIRES_TAB');const presentationId=tab.presentationId,runtimeSessionId=tab.runtimeSessionId,providerId=tab.providerId,detachScope='OWNING_SURFACE',focusContext={kind:'OPERATIONAL_TERMINAL',presentationId,runtimeSessionId,providerId},surfaceIntent={scope:detachScope,preserveRoute:true,preserveObject:true,preserveContext:true,focusedContext:clone(focusContext)};if(!this.platformWindowBridge?.requestSeparateWindow){this._record('session.detach-window',{presentationId,runtimeSessionId,providerId,detachScope,focusContext,surfaceIntent,ok:false,code:'PLATFORM_WINDOW_UNAVAILABLE',providerSessionPreserved:true});return {ok:false,code:'PLATFORM_WINDOW_UNAVAILABLE',presentationId,runtimeSessionId,providerId,detachScope,focusContext,surfaceIntent,providerSessionPreserved:true}}const result=this.platformWindowBridge.requestSeparateWindow({presentationId,runtimeSessionId,providerId,url,detachScope,focusContext,surfaceIntent});const ok=result?.ok===true&&result?.active===true;this.chrome.detached=ok;this.chrome.detachedPresentationId=ok?presentationId:null;this._record('session.detach-window',{presentationId,runtimeSessionId,providerId,detachScope,focusContext,surfaceIntent,ok,code:result?.code||null,providerSessionPreserved:true});return {...result,presentationId,runtimeSessionId,providerId,detachScope,focusContext,surfaceIntent,providerSessionPreserved:true}}
