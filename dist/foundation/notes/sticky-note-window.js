...
const direction=value=>value==='rtl'||value==='ltr'?value:null;
const firstStrong=text=>{for(const ch of String(text||'')){const cp=ch.codePointAt(0);if((cp>=0x0590&&cp<=0x08ff)||(cp>=0xfb1d&&cp<=0xfdff)||(cp>=0xfe70&&cp<=0xfefc))return 'rtl';if(/[A-Za-zÀ-ɏ]/u.test(ch))return 'ltr'}return null};
const scalarOrNull=value=>value===undefined||value===null||value===''?null:String(value);

export function resolveStickyTitleDirection({title='',persistedDirection='auto',inputDirection=null,platformDirection=null,fallbackDirection='ltr'}={}){
  const explicit=direction(persistedDirection);
  if(explicit)return Object.freeze({direction:explicit,source:'EXPLICIT_PERSISTED_DIRECTION',persistedDirection:explicit,empty:String(title||'').trim().length===0,unicodeBidi:'plaintext',technicalTokenDirection:'ltr',physicalReorder:false});
  const value=String(title||''),strong=value.trim()?firstStrong(value):null;
  if(strong)return Object.freeze({direction:strong,source:'NON_EMPTY_AUTO_FIRST_STRONG',persistedDirection:'auto',empty:false,unicodeBidi:'plaintext',technicalTokenDirection:'ltr',physicalReorder:false});
  const active=direction(inputDirection)||direction(platformDirection);
  if(active)return Object.freeze({direction:active,source:'EMPTY_ACTIVE_INPUT_PLATFORM_HINT',persistedDirection:'auto',empty:true,unicodeBidi:'plaintext',technicalTokenDirection:'ltr',physicalReorder:false});
  const fallback=direction(fallbackDirection)||'ltr';
  return Object.freeze({direction:fallback,source:'TRUTHFUL_FALLBACK',persistedDirection:'auto',empty:true,unicodeBidi:'plaintext',technicalTokenDirection:'ltr',physicalReorder:false});
}

function owningSurfaceContext(value,noteId){
  const source=value&&typeof value==='object'?value:{};
  return {noteId:String(noteId),surfaceId:scalarOrNull(source.surfaceId),domainKind:scalarOrNull(source.domainKind),route:scalarOrNull(source.route),documentId:scalarOrNull(source.documentId),objectId:scalarOrNull(source.objectId),blockId:scalarOrNull(source.blockId),sourceRange:source.sourceRange===undefined?null:clone(source.sourceRange),contextLens:scalarOrNull(source.contextLens),selectionFocus:source.selectionFocus===undefined?null:clone(source.selectionFocus),focusReturnId:scalarOrNull(source.focusReturnId)};
}
...
export class StickyNoteWindowOwner {
  ...
  register(noteId,{geometry={},open=true,pinned=false,invokerId=null,focusReturnId=null,surfaceContext=null}={}){
  ...
  setOwningSurfaceContext(noteId,context={}){const state=this._state(noteId);state.owningSurfaceContext=owningSurfaceContext({...context,focusReturnId:context.focusReturnId??state.focus.focusReturnId},state.noteId);this._record('window.owning-surface-context',{noteId:state.noteId,surfaceId:state.owningSurfaceContext.surfaceId,domainKind:state.owningSurfaceContext.domainKind,route:state.owningSurfaceContext.route,sourceBindingOwned:false,contextOwned:false});return this.window(noteId);}
  ...
  requestSeparateWindow(noteId){const state=this._state(noteId),projection=state.platform.separateWindow,detachScope='OWNING_SURFACE',context=clone(state.owningSurfaceContext),focusContext={kind:'STICKY_NOTE',noteId:state.noteId,presentationId:state.presentationId,surfaceId:context.surfaceId,objectId:context.objectId,contextLens:context.contextLens,selectionFocus:context.selectionFocus},surfaceIntent={scope:detachScope,preserveRoute:true,preserveObject:true,preserveContext:true,preserveSelection:true,preserveFocus:true,owningSurfaceContext:context,focusedContext:clone(focusContext)};projection.requested=true;if(!this.capability.separateWindow||typeof this.capability.bridge.requestSeparateWindow!=='function'){projection.capabilityAdvertised=false;projection.availability=STICKY_NOTE_PLATFORM_CAPABILITY.UNAVAILABLE;projection.requestStatus='UNAVAILABLE';projection.active=false;projection.code='UNAVAILABLE';const receipt=this._record('platform.separate-window.request',{noteId:state.noteId,detachScope,focusContext,surfaceIntent,attempted:false,ok:false,code:'UNAVAILABLE',requestStatus:'UNAVAILABLE',active:false,capabilityAdvertised:false});return {ok:false,code:'UNAVAILABLE',detachScope,focusContext,surfaceIntent,projection:clone(projection),receipt};}
    ...
    const confirmed=result.ok===true&&result.active===true&&result.contextHandoffValid===true;projection.capabilityAdvertised=true;projection.active=confirmed;projection.availability=confirmed?STICKY_NOTE_PLATFORM_CAPABILITY.ACTIVE:STICKY_NOTE_PLATFORM_CAPABILITY.DENIED;projection.requestStatus=confirmed?'SUCCEEDED':'DENIED';projection.code=confirmed?'BRIDGE_CONFIRMED':result.ok===true&&result.active===true?'DETACH_CONTEXT_HANDOFF_REQUIRED':'BRIDGE_NOT_CONFIRMED';const receipt=this._record('platform.separate-window.request',{noteId:state.noteId,detachScope,focusContext,surfaceIntent,attempted:true,ok:confirmed,code:projection.code,requestStatus:projection.requestStatus,active:projection.active,capabilityAdvertised:true,bridgeId:this.capability.bridge.id||null});return {ok:confirmed,code:projection.code,detachScope,focusContext,surfaceIntent,projection:clone(projection),receipt};}
}
