...
export class W03ScenarioDomain {
  ...
  publish({digest=null,validationContext={}}={}){if(this.lifecycle==='PUBLISHED')return freeze({ok:true,mutated:false,snapshot:this.snapshot()});const validation=this.validate(validationContext);if(!validation.ok)return freeze({ok:false,code:'SCENARIO_VALIDATION_REQUIRED_BEFORE_PUBLISH',validation,mutated:false});const frozen=freeze({...clone(this.definition),digest:digest||null});this.publishedRevisions.push(frozen);this.definition=clone(frozen);this.version++;this.lifecycle='PUBLISHED';this.validationStatus='VALIDATED';this.bindingStatus=validation.bindingStatus;return freeze({ok:true,mutated:true,snapshot:this.snapshot()})}
