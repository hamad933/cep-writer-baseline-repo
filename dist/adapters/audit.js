...
export class AuditEventDomain{
  ...
  _material(event,sequence,previousHash){return {sequence,previousHash,actor:String(event.actor||'Local product user'),action:String(event.action||'AUDIT_EVENT'),target:String(event.target||'application'),correlationId:String(event.correlationId||`audit-${sequence}`),time:String(event.time||this.clock()),outcome:String(event.outcome||'OBSERVED'),details:clone(event.details||{})};}
