...
import {BALANCED6_LEARN_SOURCE,BALANCED6_VISUALIZE_REPRESENTATIONS} from './adapters/balanced6-acceptance-data.js';
...
const learnComposition=consumer==='learn'?createLearnRuntimeComposition({source:BALANCED6_LEARN_SOURCE}):null;
...
if(isSpatial){
 ...
 stage.innerHTML=`<header class="domain-heading"><strong>${consumer==='runs'?'RUN-0042 · Internal simulation':consumer==='enterprise'?'Enterprise · v3.4 local draft':'Relationship workspace'}</strong><button class="btn" data-view="topology">Topology</button><button class="btn" data-view="table">Objects</button><button class="btn" data-view="history">History</button>${consumer==='runs'?'<button class="btn" data-view="recorded">Recorded result</button>':''}</header><div id="spatialHost" class="spatial-host"></div><div id="domainView" class="domain-readonly" hidden></div><div id="operationalHost" hidden></div>`;
...
const handleShellNavigate = async (destination        ) => {
...
if(shellNavigation){
  shellNavigation.registerContextProvider({
    ...
    restore: (ctx                     ) => {
      if (ctx?.todayFilter) {
        const filterBtn = document.querySelector             (`[data-filter="${CSS.escape(ctx.todayFilter)}"]`);
        filterBtn?.click();
      }
    }
  });
  shellNavigation.registerContextProvider({
    ...
    restore: (ctx                     ) => {
      if (ctx?.visualizeView) {
        const viewBtn = document.querySelector             (`[data-view="${CSS.escape(ctx.visualizeView)}"]`);
        viewBtn?.click();
      }
    }
  });
