...
class FakeElement{
  hidden=true;disabled=false;isConnected=true;offsetParent    ={};offsetWidth=300;offsetHeight=240;dataset    ={};style    ={};classList=new FakeClassList();children      =[];attributes=new Map               ();focused=false;
  id       ;doc    ;rect    ;
  constructor(id       ,doc    ,rect={left:20,top:20,right:70,bottom:50,width:50,height:30}){this.id=id;this.doc=doc;this.rect=rect;}
  setAttribute(k       ,v       ){this.attributes.set(k,String(v));}
  getAttribute(k       ){return this.attributes.get(k)??null;}
  removeAttribute(k       ){this.attributes.delete(k);}
  querySelector(){return null;} querySelectorAll(){return [];} addEventListener(){}
  contains(target    ){return target===this;} getBoundingClientRect(){return this.rect;}
  focus(){this.doc.activeElement=this;this.focused=true;}
}
const fakeDocument=()=>{const doc    ={activeElement:null,querySelector(){return null;}};return doc;};
const digest=(value    )=>JSON.stringify(value);

export function runPW08StructuredActionSurfaceTests(){
  const results      =[];const run=(id       ,fn        )=>{try{const detail=fn();results.push({id,status:'PASS',detail:detail??null});}catch(error    ){results.push({id,status:'FAIL',error:String(error?.stack||error)});}};
  ...
  run('PW08-T02_UNAVAILABLE_PARITY',()=>{
    const block=actions.blockMenu(context).descriptors.find((x    )=>x.actionId==='block.moveUp');
    const shift=actions.shiftF10(context).descriptors.find((x    )=>x.actionId==='block.moveUp');
  ...
  run('PW08-T06_ESCAPE_FOCUS_RETURN_G2',()=>{
    const doc=fakeDocument(),invoker=new FakeElement('invoker',doc),menu=new FakeElement('menu',doc),focus=new TransientFocusOwner({document:doc}),host=new StructuredActionSurfacePresentationHost({transientFocus:focus,document:doc,window:{innerWidth:1440,innerHeight:1000}}       );
  ...
  run('PW08-T07_OUTSIDE_DISMISS_NO_STALE_TRANSIENT',()=>{
    const doc=fakeDocument(),invoker=new FakeElement('invoker',doc),palette=new FakeElement('palette',doc),outside=new FakeElement('outside',doc),focus=new TransientFocusOwner({document:doc}),host=new StructuredActionSurfacePresentationHost({transientFocus:focus,document:doc,window:{innerWidth:1024,innerHeight:900}}       );
  ...
  run('PW08-T08_CONFIRMATION_PROTECTED_TOP',()=>{
    const doc=fakeDocument(),invoker=new FakeElement('invoker',doc),dialog=new FakeElement('dialog',doc),focus=new TransientFocusOwner({document:doc}),host=new StructuredActionSurfacePresentationHost({transientFocus:focus,document:doc,window:{innerWidth:1024,innerHeight:900}}       );
