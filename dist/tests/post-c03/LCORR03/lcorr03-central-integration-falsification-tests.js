...
const mockDocument = {
  ...
  getElementById: (id        ) => mockDocElement.querySelector(`#${id}`),
  querySelector: (sel        ) => mockDocElement.querySelector(sel),
  querySelectorAll: (sel        ) => mockDocElement.querySelectorAll(sel),
  addEventListener: () => {},
  removeEventListener: () => {}
};
...
async function test(id        , contract        , run                                  ) {
  try {
...
const standardEsc = (v     ) => String(v ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c] ));
const standardButton = (id        , label        ) => `<button class="btn" data-foundation-command="${id}">${label}</button>`;
