...
const test=async(id       ,run                             )=>{
  ...
  catch(error    ){rows.push({id,status:'FAIL',error:String(error?.stack||error)});}
};
class QueueTransport{
  responses      ;calls      =[];
  constructor(responses      =[]){this.responses=[...responses];}
  descriptor(){return {id:'LCORR01QueueTransport',semanticOwnership:false};}
  async request(method       ,path       ,body    ){this.calls.push({method,path,body});return structuredClone(this.responses.shift()??{ok:false,code:'NO_RESPONSE'});}
}
...
await test('A15-PF-003.reviews-complete-lifecycle-is-command-reachable',()=>{
  const domain=new W04ReviewDomain([],{evidenceResolver:()=>({state:'RESOLVED',immutable:true}),reviewAuthorityRegistry:{resolveReviewerAuthority:(identity       )=>({identity,authorized:true,canAssign:true,permissionProofRef:`proof:${identity}`,testOnly:false})}});
...
await test('A15-PF-004.empty-portfolio-adds-exact-reference-without-copy',()=>{
  const domain=new W04PortfolioDomain([],{sourceResolver:{inspect:(ref       )=>ref==='evidence-1@r1'?{digest:'sha256:source',rowCount:1}:null}});
...
await test('A13-PF-004.scenario-publish-command-reachable-and-resolver-gated',()=>{
  ...
  const context={resolveLab:(ref    )=>ref.id==='LAB'&&ref.revision==='1',availableCapabilities:[],binding:{capabilities:[]}};
...
await test('A07-PF-001.detach-requires-provider-reattach-capability',()=>{
  const runtime={descriptor:()=>({contract:{id:'RuntimeAdapter',version:'1.0.0'},id:'NoReattachRuntime',label:'No reattach',sessionOwner:'RuntimeOwner',capabilities:[]}),session:(id       )=>({id,deviceId:'device',epoch:'e1',lines:[],presentation:{state:'running',detail:'',sequence:1,transitions:[]}}),prompt:()=>''};
...
await test('A12-PF-007.visualize-available-hierarchy-renders-provider-nodes',()=>{
  assert.equal(typeof (visualizeSurface       ).renderVisualizeHierarchyTree,'function');
  const html=(visualizeSurface       ).renderVisualizeHierarchyTree([{id:'root',label:'Root',children:[{id:'child',label:'Child'}]}],new Set(['child']));
...
await test('CORR-03.scenarios-publish-availability-truth',()=>{
  ...
  const validContext={
    resolveLab:(ref    )=>ref.id==='LAB-REQ'&&ref.revision==='1',
...
await test('RC-05.xterm-reconciliation-ten-contracts',async()=>{
  class MockTerminal{
    ...
    options    ={};
    open(host    ){}
    write(data    ){this.written+=typeof data==='string'?data:Buffer.from(data).toString();}
    writeln(data    ){this.written+=String(data)+'\n';}
