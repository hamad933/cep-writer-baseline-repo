...
await test('d09.results.timeline-replay-shared-owner-reuse-and-no-second-engine', () => {
  ...
  try {
    new (W03ResultsDomain       )({records: makeSampleResults(), analyticalCompareOwner});
  } catch (err     ) {
...
await test('d09.results.analytical-compare-shared-owner-reuse', () => {
  ...
  let thrownMissing = false;
  try {
    new (W03ResultsDomain       )({records: makeSampleResults(), timelineReplayOwner});
  } catch (err     ) {
...
await test('d09.scenarios-labs.lifecycle-preflight-and-resolver-unbound-guards', () => {
  ...
  assert(val.requirements.some((x     ) => x.code === 'LAB_REVISION_RESOLVER_UNBOUND'), 'reports LAB_REVISION_RESOLVER_UNBOUND');
