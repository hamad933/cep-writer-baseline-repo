...
const run=(name       ,fn         ,out      )=>{try{fn();out.push({name,status:'PASS'})}catch(e){out.push({name,status:'FAIL',error:String((e       )?.message||e)})}};
const throws=(fn         ,code       )=>{let caught    ;try{fn()}catch(e){caught=String((e       )?.message||e)}assert(typeof caught==='string'&&caught.includes(code),`expected ${code}, got ${caught}`)};

const memoryStorage=(fail=false)    =>({data:{}                         ,getItem(k       ){return this.data[k]??null},setItem(k       ,v       ){if(fail)throw Error('WRITE_DENIED');this.data[k]=String(v)}});
const makeSettings=(storage    =null,bus=new SemanticCommandBus())=>{const preferences=new ScopedPreferencesOwner(storage),keymap=new GlobalInputKeymapOwner({commands:bus}),transientFocus=new TransientFocusOwner();return {settings:new SettingsCenterOwner({preferences,commands:bus,keymap,transientFocus}),preferences,bus}};

export function runD03ACommandSettingsConvergenceTests(){const t      =[];
  ...
  run('d03a.negative.composition-requires-canonical-bus-instance',()=>{
    ...
    throws(()=>createManualAiSurfaceComposition({commands:{items:()=>[]}       }),'CANONICAL_SEMANTIC_COMMAND_BUS_REQUIRED');
  ...
  run('d03a.settings-owner-uniqueness-canonical-identities',()=>{
    ...
    assert(settings.sections().filter((s    )=>s.id===settings.contract.sc011PreferenceTransferActionHome).length===1,'exactly one SC-011 transfer action-home');
  },t);
  run('d03a.sc011-exposure-through-canonical-settings',()=>{
    const {settings}=makeSettings(memoryStorage());
    const section=settings.sections().find((s    )=>s.id==='settings.transfer');
    ...
    const actionIds=section.items.map((i    )=>i.actionId);
    ...
    assert(section.items.every((i    )=>i.delegatesTo===SCOPED_PREFERENCES_OWNER&&i.kind==='preference-transfer-action'),'actions declare delegation to canonical preference owner');
    assert(settings.search('export').some((h    )=>h.sectionId==='settings.transfer'),'Settings search must reach the SC-011 action-home');
  ...
  run('d03a.sc011-failing-storage-import-reported',()=>{
    const store    ={data:{}                         ,getItem(k       ){return this.data[k]??null},setItem(){throw Error('WRITE_DENIED')}};
...
if(import.meta.url===new URL(process.argv[1],'file:').href){const tests=runD03ACommandSettingsConvergenceTests();const report={mission:'D03A',kind:'MODEL_NOT_BROWSER',pass:tests.filter((x    )=>x.status==='PASS').length,fail:tests.filter((x    )=>x.status==='FAIL').length,tests};console.log(JSON.stringify(report,null,2));if(tests.some((x    )=>x.status==='FAIL'))process.exitCode=1;}
