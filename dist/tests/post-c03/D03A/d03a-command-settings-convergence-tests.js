...
if(process.argv[1]&&(import.meta.url.includes(process.argv[1].replace(/^[A-Za-z]:/,'').replaceAll('\\','/'))||process.argv[1].includes('d03a-command-settings-convergence-tests'))){const tests=runD03ACommandSettingsConvergenceTests();const report={mission:'D03A',kind:'MODEL_NOT_BROWSER',pass:tests.filter((x    )=>x.status==='PASS').length,fail:tests.filter((x    )=>x.status==='FAIL').length,tests};console.log(JSON.stringify(report,null,2));if(tests.some((x    )=>x.status==='FAIL'))process.exitCode=1;}
