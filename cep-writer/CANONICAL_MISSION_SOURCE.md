...
Read the current Controller finding ledger `R6_PRESENTATION_CORR02_PRE_WRITER_DEEP_DEFECT_AUDIT.md` Drive `1VFEr8f8ptCaJuxeApcnXD-dpWRkqucCW`, including F-001..F-041. Fresh runtime evidence:
...
4. close F-005/F-021/F-033 region-binding defects by binding Profile-defined LEFT/CENTER/RIGHT/BOTTOM/TOOLBAR roles into the existing shared hosts rather than rendering right/bottom substitutes inside CENTER;
5. preserve the existing responsive pane mechanism; final genuine-route test must keep `22/22` RIGHT reveal behavior at `1024×900`;
6. prohibit new consumer-specific CSS masks as the primary fix. Existing compatibility masks may remain only when explicitly classified temporary and no longer required for semantic correctness.
...
**C2 — W04 Product data/action truth.** Starts only from clean C1. Close F-022..F-026/F-038 without inventing authority/provider facts.

**C3 — Today provider/epistemic truth.** Starts only from clean C2 or from the exact C1 parent if Controller proves no W04 dependency. Close F-007..F-011 and Today-specific region/data truth.
