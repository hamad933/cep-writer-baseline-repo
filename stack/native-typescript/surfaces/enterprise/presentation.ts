...
export function renderEnterpriseSurface(root,composition,{dir='ltr',spatialView=null}={}){
  ...
  const draw=({preserveSpatial=false}={})=>{
    ...
    root.querySelector('.enterprise-identity')?.insertAdjacentHTML('afterend',[commandButton('enterprise.create','Create workspace',composition),commandButton('enterprise.baseline','Pin exact Baseline',composition),commandButton('enterprise.validate','Validate',composition),commandButton('enterprise.publish','Publish revision',composition)].join(''));
    ...
    root.querySelector('[data-command="enterprise.baseline"]')?.addEventListener('click',()=>{mode='baselines';draw();const main=root.querySelector('.enterprise-main');if(!main)return;const form=document.createElement('form');form.className='enterprise-edit-form enterprise-summary-card';form.dataset.baselineComposer='';form.innerHTML='<label>Baseline ID<input name="id" required></label><label>Revision<input name="revision" required></label><label class="full">Exact digest<input name="digest" required></label><button class="btn" type="submit">Pin exact Baseline</button><button class="btn" type="button" data-cancel-baseline>Cancel</button>';main.prepend(form);form.addEventListener('submit',event=>{event.preventDefault();const data=new FormData(form),result=composition.bus.execute('enterprise.baseline',{baseline:{status:'AVAILABLE',id:String(data.get('id')||''),revision:String(data.get('revision')||''),digest:String(data.get('digest')||'')}});if(result?.ok===false)setStatus(result.code,'error');else{status='Exact Baseline identity pinned.';draw()}});form.querySelector('[data-cancel-baseline]')?.addEventListener('click',()=>form.remove())});
