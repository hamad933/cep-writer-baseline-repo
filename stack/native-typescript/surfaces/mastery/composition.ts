...
export function createMasterySurfaceComposition({domain=new W04MasteryDomain(),analyticalCompareOwner=null}={}){
 const tableAdapter:CollectionTableMatrixDomainAdapter<any>={
  ...
  actions:()=>[{id:'mastery.inspect',label:'Inspect',enabled:true},{id:'mastery.explain',label:'Explain',enabled:true},{id:'mastery.reevaluate',label:'Request governed re-evaluation',enabled:true}]
 };
 ...
 const centerProjection=(id=domain.records[0]?.id)=>{
  if(!id)return {state:'EMPTY',kind:'ExplainabilityProjectionWorkbench',message:'No governed Mastery State is available.',authority:{institutionalAuthorityInferred:false,authorizedEvaluatorBound:!!domain.evaluatorDescriptor}};
  const row=domain.inspect(id),explanation=domain.explain(id);
  return {state:'READY',kind:'ExplainabilityProjectionWorkbench',identity:{id:row.id,revisionId:row.revisionId,subject:row.subject,masteryTarget:row.capability},judgment:{value:row.judgment},freshness:{value:row.freshness,independentFromJudgment:true},governedBasis:{policyRef:row.policyRef,basisDigest:row.basis.digest,effectiveDecisionRefs:[...row.basis.decisionRefs],evidenceRevisionRefs:[...row.basis.evidenceRefs]},causalLaw:explanation.causalLaw,completionOrActivityUsed:false,authority:{institutionalAuthorityInferred:false,authorizedEvaluatorBound:!!domain.evaluatorDescriptor,reevaluateMode:'REQUEST_OR_INSPECT_ONLY',localCanonicalWrite:false}};
 };
 return {surface:MASTERY_SURFACE_ID,workspaceBinding:createFamilyWorkspaceBinding({id:'mastery.workspace',family:'global',domainKind:'mastery',label:'Mastery'}),domain,collection,tableAdapter,compareOwner,compareProvider,provenance,provenanceProvider,contextProvider,centerProjection,toolbarCommandIds:['mastery.inspect','mastery.explain','mastery.reevaluate'],bottomProjection:(id=domain.records[0]?.id)=>id?{history:domain.history.filter(item=>item.recordId===id),provenance:provenanceProvider.read({id}),diagnostics:{canonicalWriterBound:false,authorizedEvaluatorBound:!!domain.evaluatorDescriptor,reevaluateLocalWrite:false,judgmentFreshnessIndependent:true}}:{history:[],provenance:provenanceProvider.read({}),diagnostics:{canonicalWriterBound:false,authorizedEvaluatorBound:!!domain.evaluatorDescriptor,reevaluateLocalWrite:false}},slots:{TOP:'shared',TOOLBAR:'shared',LEFT:'collection',CENTER:'ExplainabilityProjectionWorkbench',RIGHT:'shared-context-inspector',BOTTOM:'shared-bottom-shell/domain-projection',TRANSIENT:'shared'},truth:{masteryFromCompletion:false,canonicalMasteryWriter:'EXTERNAL_AUTHORITY_ONLY',reevaluate:'REQUEST_INSPECT_ONLY__ZERO_LOCAL_MASTERY_STATE_WRITE',judgmentFreshnessIndependent:true,formalReviewAuthority:false}};
}
