...
const tests:any[]=[];const run=(name,fn)=>{try{fn();tests.push({name,status:'PASS'})}catch(e){tests.push({name,status:'FAIL',error:String(e?.message||e)})}};const assert=(v,m)=>{if(!v)throw Error(m)};
