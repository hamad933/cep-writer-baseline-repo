...
assert.equal(source.includes('-SUCCESSOR'),false,'Enterprise presentation must not fabricate successor Baseline IDs');
assert.equal(source.includes(':successor'),false,'Enterprise presentation must not fabricate successor Baseline digests');
assert.equal(source.includes("setTwinBinding({baselineStatus:'STALE'})"),false,'Enterprise presentation must not fabricate provider staleness');
