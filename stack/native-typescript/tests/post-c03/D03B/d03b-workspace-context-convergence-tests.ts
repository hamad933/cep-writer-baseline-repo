...
import type {ShellSemanticContextProvider} from '../../../foundation/global/shell/navigation.js';

const assert=(v:any,m='assertion failed')=>{if(!v)throw Error(m)};
const run=(name:string,fn:()=>void,out:any[])=>{try{fn();out.push({name,status:'PASS'})}catch(e){out.push({name,status:'FAIL',error:String((e as any)?.stack||(e as any)?.message||e)})}};
const throws=(fn:()=>void,code:string)=>{let caught:any;try{fn()}catch(e){caught=String((e as any)?.message||e)}assert(typeof caught==='string'&&caught.includes(code),expected , got )};

const mockElement=(id:string,tag='div'):any=>{
  ...
  return {
    ...
    closest(sel:string){return sel.includes(id)?this:null}
  };
};

export function runD03BWorkspaceContextConvergenceTests(){const t:any[]=[];
  ...
  run('d03b.region-cycle-f6-delegation-canonical-input-owner',()=>{
    ...
    assert(r5.receipt.skipped.some((s:any)=>s.id==='RIGHT'&&s.reasons.includes('unavailable')),'unavailable region was skipped with receipt proof');
  ...
  run('d03b.semantic-surface-context-provider-today-and-visualize',()=>{
    const registry=new Map<string, ShellSemanticContextProvider>();
    const register=(p:ShellSemanticContextProvider)=>registry.set(p.surface,p);

    let todayFilter='ALL';
    const todayElement=mockElement('todayView');
    const todayProvider:ShellSemanticContextProvider={
      surface:'today',
    ...
    const visualizeProvider:ShellSemanticContextProvider={
      surface:'visualize',
...
if(process.argv[1]&&(import.meta.url.includes(process.argv[1].replace(/^[A-Za-z]:/,'').replaceAll('\\','/'))||process.argv[1].includes('d03b-workspace-context-convergence-tests'))){
  ...
  const report={mission:'D03B',kind:'MODEL_NOT_BROWSER',pass:tests.filter((x:any)=>x.status==='PASS').length,fail:tests.filter((x:any)=>x.status==='FAIL').length,tests};
