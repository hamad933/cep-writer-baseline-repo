...
class MockTransport {
  ...
  responses: Array<any | ((req: any) => any)> = [];

  constructor(responses: Array<any | ((req: any) => any)> = []) {
...
const mockAttempt = (number: number, state: string, providerEvidence: any = null) => ({
...
const mockJob = (state = 'FAILED', attempts = [mockAttempt(1, 'FAILED')], extra: any = {}) => ({
  jobId: 'job-1',
  requestId: 'req-1',
...
// 2.1 Exact artifact identity, ruleset binding, and technical validity boundaries
{
  ...
  assert.equal(missingRes.technicalFindings.some((f: any) => f.code === 'VALIDATOR_IDENTITY_MISSING'), true);
  ...
  assert.equal(findings.technicalFindings.every((f: any) => f.formalReviewFinding === false), true);
...
const testCandidate = (id: string, patch: any = {}): ReleaseCandidate => ({
  candidateId: id,
...
// 4.2 Configuration: proposal containment, zero live mutation, sensitive key exclusion, global settings boundary
{
  ...
  (cfgAdapter as any).observations[0].presentVersion = 'v2';
...
// 4.3 Canonical SemanticCommandBus DI Contract Falsification (D03A)
{
  ...
  assert.throws(
    () => createManualAiSurfaceComposition({commands: null as any}),
    /CANONICAL_SEMANTIC_COMMAND_BUS_REQUIRED/
  );
  assert.throws(
    () => createReleasesSurfaceComposition({commands: undefined as any}),
    /CANONICAL_SEMANTIC_COMMAND_BUS_REQUIRED/
  );
  assert.throws(
    () => createConfigurationSurfaceComposition({commands: {} as any}),
    /CANONICAL_SEMANTIC_COMMAND_BUS_REQUIRED/
  );
