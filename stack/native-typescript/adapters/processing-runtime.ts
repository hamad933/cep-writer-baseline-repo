...
const availabilityEnabled=value=>value===true||Boolean(value&&typeof value==='object'&&value.enabled===true);
...
export class ProcessingRuntimeAdapter{
  constructor({transport=createBoundedLocalRuntimeTransport()}={}){
    ...
    this.tableAdapter={
      ...
      actions:row=>[{id:'processing.inspect',label:'Inspect',enabled:availabilityEnabled(this.availability('processing.inspect',{jobId:row.jobId}))},{id:'processing.retry',label:'Retry',enabled:availabilityEnabled(this.availability('processing.retry',{jobId:row.jobId}))},{id:'processing.requestCancel',label:'Request cancel',enabled:availabilityEnabled(this.availability('processing.requestCancel',{jobId:row.jobId}))},{id:'processing.validationHandoff',label:'Validation handoff',enabled:availabilityEnabled(this.availability('processing.validationHandoff',{jobId:row.jobId}))}]
    };
  ...
  availability(id,payload={}){
    ...
    if(id==='processing.requestCancel'){if(!job)return 'Select a non-terminal Job';if(terminalStates.has(job.state))return 'Select a non-terminal Job';if(job.state==='CANCEL_REQUESTED'||job?.cancellation?.state==='REQUESTED')return 'Cancellation already requested';return true;}
    if(id==='processing.validationHandoff'){if(job?.state!=='COMPLETED')return 'A COMPLETED Job is required';if(job?.validationHandoff)return `Validation handoff already ${job.validationHandoff.state||'requested'}`;return true;}
  ...
  mount({stage,registry,workspace,button,esc}){
    const render=()=>{
      ...
      const providerLabel=state.refreshPhase==='FETCHING'?'REFRESHING':state.providerState==='AVAILABLE'?'PROVIDER AVAILABLE':state.providerState==='ERROR'?'PROVIDER ERROR':'PROVIDER UNAVAILABLE';
      const providerTone=state.refreshPhase==='FETCHING'?'loading':state.providerState==='AVAILABLE'?'success':state.providerState==='ERROR'?'error':'unavailable';
      ...
      <div data-w05-surface="processing">
        <header class="w05-processing-head" dir="ltr"><div><div class="m0-eyebrow">W05 · Pipeline lifecycle</div><h1>Processing</h1><p>Job, Attempt, retry, cancellation and validation-handoff state are shown separately. No provider or consumer success is inferred.</p></div><span class="state-token" data-processing-provider-state="${esc(state.providerState)}" data-state="${esc(providerTone)}">${esc(providerLabel)}</span></header>
        ${state.refreshPhase!=='FETCHING'&&state.providerState!=='AVAILABLE'?`<p class="state-token" data-processing-provider-notice data-state="${esc(providerTone)}" dir="ltr">Provider ${esc(state.providerState)} · retained Jobs are last-known inspection data only. Retry, cancellation request and validation handoff mutations are unavailable until a successful provider refresh.</p>`:''}
