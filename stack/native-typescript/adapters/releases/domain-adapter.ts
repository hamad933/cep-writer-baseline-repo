...
export type ReleaseEvidenceBinding={digest:string;candidateRef:ReleaseCandidateRef;method:string;evidenceRefs?:string[]};
export type ReleaseCandidate={candidateId:string;commitSHA:string;treeSHA:string;artifactDigest:string;state:CandidateState;evidenceDigest:string|null;evidenceBinding?:ReleaseEvidenceBinding|null;authorization:AuthorizationState;deployment:DeploymentState;deploymentObservedAt:string|null};
...
const normalizeEvidence=(row:ReleaseCandidate):ReleaseEvidenceBinding|null=>{
  ...
  const binding:satisfies<ReleaseEvidenceBinding>=supplied?clone(supplied):{digest:row.evidenceDigest,candidateRef:{candidateId:row.candidateId,commitSHA:row.commitSHA,treeSHA:row.treeSHA,artifactDigest:row.artifactDigest},method:'INLINE_EXACT_CANDIDATE_RECORD',evidenceRefs:[]};
...
export class ReleasesDomainAdapter{
  readonly owner=RELEASES_DOMAIN_OWNER;private candidates=new Map<string,ReleaseCandidate>();private selectedId:string|null=null;private requester:AuthorizationRequester|null;private history:any[]=[];readonly compareOwner:AnalyticalCompareOwner;private compareProvider:any;
