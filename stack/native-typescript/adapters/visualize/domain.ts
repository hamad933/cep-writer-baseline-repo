...
export class VisualizeDomainAdapter{
  ...
  providerTruth(){
    const descriptor=this.providerDescriptor(),projection=this.canonicalProjection(),relationWritable=typeof this.provider?.validateRelation==='function'&&typeof this.provider?.commitRelation==='function',objectWritable=typeof this.provider?.editObject==='function',localAcceptance=descriptor.authority==='LOCAL_ACCEPTANCE_PROJECTION_ONLY';
    return {providerId:descriptor.providerId||'UNKNOWN',authority:descriptor.authority||'UNSPECIFIED',projectionStatus:projection.status||'UNAVAILABLE',canonical:projection.ok===true&&projection.canonical===true,relationMutation:!localAcceptance&&relationWritable?'AVAILABLE_BY_PROVIDER_CONTRACT':'READ_ONLY',objectEdit:!localAcceptance&&objectWritable?'AVAILABLE_BY_PROVIDER_CONTRACT':'READ_ONLY',editability:!localAcceptance&&(relationWritable||objectWritable)?'PROVIDER_COMMAND_BOUND':'READ_ONLY',representationTruth:projection.ok?(projection.canonical===true?'REPRESENTATION_OF_CANONICAL_PROVIDER':'LOCAL_ACCEPTANCE_REPRESENTATION_ONLY'):'SYNTHETIC_REPRESENTATION_ONLY'};
  }
  ...
  editAvailability(payload={}){if(!this.provider)return {enabled:false,code:'VISUALIZE_CANONICAL_DATA_PROVIDER_UNAVAILABLE',reason:'Canonical provider is unavailable.'};if(this.providerDescriptor().authority==='LOCAL_ACCEPTANCE_PROJECTION_ONLY')return {enabled:false,code:'VISUALIZE_LOCAL_ACCEPTANCE_PROVIDER_READ_ONLY',reason:'The bound local acceptance projection is canonical:false and read-only; canonical object editing is unavailable.'};if(typeof this.provider.editObject!=='function')return {enabled:false,code:'VISUALIZE_CANONICAL_EDIT_COMMAND_REQUIRED',reason:'Canonical object edit requires a bound provider command.'};if(!String(payload.objectId||''))return {enabled:false,code:'VISUALIZE_CANONICAL_OBJECT_REQUIRED',reason:'Canonical object identity is required.'};return {enabled:true,code:'AVAILABLE',reason:''};}
