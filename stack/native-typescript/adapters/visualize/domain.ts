...
export class VisualizeDomainAdapter{
  constructor({provider=null,representations=[],spatialModel=null}={}){
    ...
    const seedNodes=this.representations.map(item=>({id:item.representationId,x:finite(item.x)?Number(item.x):0,y:finite(item.y)?Number(item.y):0,label:item.label||item.representationId,type:item.type||'representation'}));
    if(spatialModel){
      ...
      const ids=new Set(spatialModel.nodes.map(node=>String(node.id)));for(const node of seedNodes)if(!ids.has(node.id))throw Error(`VISUALIZE_SHARED_SPATIAL_MODEL_MISSING_REPRESENTATION:${node.id}`);
      this.model=spatialModel;this.sharedSpatialModelBound=true;
  ...
  activateView(mode){const key=modeOf(mode);if(!key)return {ok:false,status:'VISUALIZE_VIEW_MODE_REQUIRED'};const capabilities=VIEW_CAPABILITIES[key];this.model.setActiveMode(capabilities.spatialMode);return {ok:true,status:'VISUALIZE_VIEW_ACTIVE',mode:key,spatialMode:capabilities.spatialMode,capabilities:clone(capabilities),engineOwner:this.model.interactionKernel.ownerId,canonicalMutation:false};}
  canonicalProjection(){
  ...
  moveAvailability({mode,representationIds=[],dx,dy}={}){const key=modeOf(mode);if(!key)return {enabled:false,code:'VISUALIZE_VIEW_MODE_REQUIRED',reason:'Tree/Path/Graph/Canvas view context is required.'};if(!VIEW_CAPABILITIES[key].move)return {enabled:false,code:'VISUALIZE_MOVE_REQUIRES_CANVAS_VIEW',reason:'Representation geometry can be moved only in Canvas view.',mode:key};const checked=this.view(key).assertIds(representationIds);if(!checked.ok)return {enabled:false,code:checked.status,reason:'Selection contains a representation outside the active view.',mode:key};if(!checked.ids.length)return {enabled:false,code:'VISUALIZE_MOVE_SELECTION_REQUIRED',reason:'Select at least one Canvas representation before moving it.',mode:key};if(dx!==undefined&&!finite(dx)||dy!==undefined&&!finite(dy))return {enabled:false,code:'VISUALIZE_NON_FINITE_GEOMETRY_REJECTED',reason:'Move geometry must be finite.',mode:key};const spatial=this.model.geometryMutationAvailability({minimumSelection:1,action:'visualize.move'});return spatial.enabled?{enabled:true,code:'AVAILABLE',reason:'',mode:key}:{enabled:false,code:spatial.code,reason:spatial.reason,mode:key};}
  ...
  linkAvailability(payload={}){
    ...
    if(this.providerDescriptor().authority==='LOCAL_ACCEPTANCE_PROJECTION_ONLY')return {enabled:false,code:'VISUALIZE_LOCAL_ACCEPTANCE_PROVIDER_READ_ONLY',reason:'The bound local acceptance projection is canonical:false and read-only; relation mutation is unavailable.'};
    const projection=this.canonicalProjection();if(projection.ok!==true||projection.canonical!==true)return {enabled:false,code:'VISUALIZE_CANONICAL_PROVIDER_REQUIRED',reason:'Relation mutation requires a bound provider projection that explicitly proves canonical:true.'};
