...
def run_flow(browser,definition,fn,target=flows):
    ...
    if _SHARED_CTX is None:
        _SHARED_CTX=browser.new_context(viewport={'width':1440,'height':980},reduced_motion='reduce')
        _SHARED_PAGE=_SHARED_CTX.new_page(); _SHARED_PAGE.set_default_timeout(5000)
    page=_SHARED_PAGE; errs=[]
    def onerr(e): errs.append(str(e))
    page.on('pageerror',onerr)
    print('START',definition['id'],flush=True)
    try:
        ...
        try: page.remove_listener('pageerror',onerr)
...
with sync_playwright() as p:
    ...
    if _SHARED_CTX is not None: _SHARED_CTX.close()
    browser.close()
