...
class CDP:
 def __init__(self):
  ...
  self.ws=None;self.target_id=None;self.seq=0;self.new_target()
 def new_target(self):
  ...
  self.target_id=target['id'];self.seq=0
 ...
 def load(self,surface,w,h):
  # Fresh target per route load prevents singleton/global leakage between genuine
  # consumers while still executing the exact same built ESM bytes.
  self.new_target()
...
try:
    ...
    def f5():
        ...
        time.sleep(.35)
        xtermReady=cdp.ev("!!document.querySelector('#operationalHost .xterm-helper-textarea')")
        if not xtermReady: raise AssertionError('xterm input not mounted')
        cdp.ev("document.querySelector('#operationalHost .xterm-helper-textarea').focus();true")
        for ch in 'shutdown':
            vk=ord(ch.upper());cdp.cmd('Input.dispatchKeyEvent',{'type':'keyDown','key':ch,'code':'Key'+ch.upper(),'windowsVirtualKeyCode':vk,'nativeVirtualKeyCode':vk,'text':ch,'unmodifiedText':ch});cdp.cmd('Input.dispatchKeyEvent',{'type':'keyUp','key':ch,'code':'Key'+ch.upper(),'windowsVirtualKeyCode':vk,'nativeVirtualKeyCode':vk})
        key_mod('Enter');time.sleep(.35)
        state=cdp.ev("(()=>{const d=CEPFoundation.simulation.devices.find(x=>x.id==='DEV-WEB-01'),n=CEPFoundation.spatial.model.nodes.find(x=>x.id==='DEV-WEB-01'),e=CEPFoundation.simulation.events.at(-1),r=CEPFoundation.simulation.recorded().devices.find(x=>x.id==='DEV-WEB-01');return{up:d.up,nodeStatus:n.status,semanticCommand:e.semanticCommand,eventOutput:e.output,recordedUp:r.up,provider:CEPFoundation.wave4Assembly.operationalSession.activeTab()?.providerId||null,terminalText:document.querySelector('#operationalHost .xterm-rows')?.innerText||document.querySelector('#operationalHost .terminal-output')?.innerText||''}})()")
    ...
    def f6():
        ...
        iso=cdp.ev("({hostKind:CEPFoundation.api.hostKind,domainKind:CEPFoundation.structured.domainKind,libraryState:CEPFoundation.api.state.library??null,searchCount:document.querySelectorAll('.library-search').length,bdi:[...document.querySelectorAll('[dir=\"ltr\"]')].some(n=>String(n.textContent||'').trim().length>0),isolation:CEPFoundation.structured.assertDomainIsolation().ok})")
    ...
    report={'schemaVersion':1,'classification':'REPRODUCIBLE_BROWSER_CONFORMANCE_RECEIPT_NOT_OWNER_ACCEPTANCE','command':'python tools/r6-browser-conformance-cdp.py','sourceFoundationBaseline':runtime['baselineId'],'sourceCandidate':f"CANONICAL_SOURCE_TREE_SHA256:{identity['sha256']}",'sourceCanonicalTreeSha256':identity['sha256'],'canonicalSourceFileCount':identity['files'],'currentUse':'EXACT_CURRENT_CANONICAL_SOURCE_EXECUTION_RECEIPT','executionStatus':'EXECUTED_PASS' if summary=={'total':6,'pass':6,'fail':0} else 'BLOCKED_OR_FAILED','prerequisite':'SATISFIED: Chromium DevTools Protocol launched /usr/bin/chromium and executed the exact current built ESM graph through in-memory Blob module URLs because localhost navigation is administratively blocked.','declaredFlows':[x['id'] for x in flows],'environment':{'engine':'Chromium CDP','executable':'/usr/bin/chromium','transport':'in-memory-built-esm-blob-urls','moduleCount':len(mods),'viewport':'1440x980','reducedMotion':True},'summary':summary,'flows':flows,'screenshots':shots,'limitations':['Headless Chromium only','Six bounded critical flows; complemented by R6 23-route audit','In-memory transport uses exact current built bytes because localhost navigation is administratively blocked; xterm module is injected from exact built dist/vendor/xterm/xterm.mjs']}
